...
/**
 * Check if user can view prep tasks
 * @param {string} role - User role
 * @returns {boolean} - Whether user can view prep
 */
...
/**
 * Check if user can edit prep tasks
 * @param {string} role - User role
 * @returns {boolean} - Whether user can edit prep
 */
export function canEditPrep(role) {
  // Members can check/uncheck tasks (edit their status)
  // Editors, admins, owners can do full editing
  return [ROLES.OWNER, ROLES.ADMIN, ROLES.EDITOR, ROLES.MEMBER].includes(role);
}

/**
 * Check if user can access specific admin tab
 * @param {string} role - User role
...
export function canAccessAdminTab(role, tab) {
  ...
  // Editor can access: info, menu, recipes, prep
  if (role === ROLES.EDITOR) {
    return ['info', 'menu', 'recipes', 'prep'].includes(tab);
  }
  
  // Member can access: info, prep
  if (role === ROLES.MEMBER) {
    return ['info', 'prep'].includes(tab);
  }
