...
-- Create role permissions table (optional, for future extension)
...
-- Insert default permissions (only if the table is still empty, so this stays re-runnable)
...
CREATE INDEX IF NOT EXISTS idx_staff_company_role ON restaurant_staff(company_id, role);
